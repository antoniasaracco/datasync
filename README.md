...
1. validates the source against a supplied MD5 and/or SHA-256 checksum manifest with the [`rclone_checksum` module](modules/nf-core/rclone/checksum), which wraps [`rclone checksum`](https://rclone.org/commands/rclone_checksum/);
...
- `rclone/copy/`, `rclone/checksum/<sample>/`, and `rclone/check/<sample>/` contain copy logs, pre-copy checksum reports, and post-copy integrity status files;
