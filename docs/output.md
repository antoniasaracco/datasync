...
Both stages use the same `<sample>.*.txt` naming convention, but they are published to separate stage-specific directories. This keeps pre-copy checksum validation reports separate from post-copy destination validation reports for each samplesheet row.
...
- checksum validation status for MD5 and/or SHA-256 manifests generated from `rclone checksum` / the `RCLONE_CHECKSUM` step;
- post-copy source-to-destination validation status generated from `rclone check` / the `RCLONE_CHECK` step;
...
> **Screenshot placeholder:** Add a screenshot at `docs/images/multiqc_rclone_checksum_section.png` showing the MultiQC MD5/SHA-256 input-validation table with representative match, mismatch, missing, and error rows.
...
> **Screenshot placeholder:** Add a screenshot at `docs/images/multiqc_rclone_check_section.png` showing the MultiQC source-destination validation table with representative match, mismatch, missing, and error rows.
